...
public class NaturalCubicSpline {
	/**
	 * Solve a tri-diagonal set of equations.
	 ...
	 * Note that the input arrays will be modified by this function.
	 * @param va Lower diagonal vector.
	 * @param vd Main diagonal vector. 
	 * @param vc Upper diagonal vector.
	 * @param vb The right-hand vector.
	 * @return A solution vector (x) for the matrix.
	 */
	private static double[] solveTriDiagonalEquation(
			double[] va, double[] vd, double[] vc, double[] vb) {
		int size = va.length;
	...
	/**
	 * Calculate the natural spline coefficients of the current points.
	 * The coefficients consist of four arrays A, B, C and D so that
	 * p(x) = A[i] * (x-X[i])^3 + B[i] * (x-X[i]]^2 + C[i] * (x-X[i]) + D[i]
	 *   where X[i] <= x <= X[i+i]
	 *   and X[1..n+1] is the sorted array of x values in the control points.)  
	...
	public double[] getSplineValues(double startX, double endX, int steps) {
		...
		for (int step=0; step<steps; step++) {
			...
			double y = coefficients.a[cofIndex] * hhh +
				coefficients.b[cofIndex] * hh +
				coefficients.c[cofIndex] * h +
				coefficients.d[cofIndex];
			values[step] = y;
		}
		return values;
	}
	...
	public double old_getSplineValue(double x) {
