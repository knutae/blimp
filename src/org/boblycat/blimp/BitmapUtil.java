...
public class BitmapUtil {
	private static byte[] getShortRGBData(RGB48Image rgb,
	...
	private static byte[] getByteRGBData(RGB24Image rgb,
			int index1, int index2, int index3) {
        int width = rgb.getWidth();
        int height = rgb.getHeight();
        byte[] bytes = new byte[width*height*3];
	...
	private static byte[] getRGBData(PixelImage pixelImage,
			int index1, int index2, int index3) {
		if (pixelImage instanceof RGB48Image)
			return getShortRGBData((RGB48Image) pixelImage, index1, index2, index3);
		if (pixelImage instanceof RGB24Image)
			return getByteRGBData((RGB24Image) pixelImage, index1, index2, index3);
		return null;
	}
	 ...
	 * @param pixelImage The image to convert.
	 * @return A byte array of BGR triplets, or <code>null</code> if the image type
	 * is not supported.  The size of the array will be width*height*3. 
	 */
	public static byte[] get8BitBGRData(PixelImage pixelImage) {
		return getRGBData(pixelImage,
				RGBIndex.INDEX_BLUE,
	 ...
	 * least significant byte will be dropped.
	 * @param pixelImage The image to convert.
	 * @return A byte array of RGB triplets, or <code>null</code> if the image type
	 * is not supported.  The size of the array will be width*height*3. 
	 */
	...
	private static BufferedImage toAwtImage(PixelImage pixelImage) {
		...
		awtImage.getRaster().setDataElements(0, 0,
				pixelImage.getWidth(), pixelImage.getHeight(), bytes);
		return awtImage;
	}

	public static void writeBitmap(Bitmap bitmap, String formatName,
			String fileName, double quality)
	throws IOException {
		ImageWriter writer = getImageWriter(formatName);
		if (writer == null) {
			System.err.println("Failed to get image writer for format: " + formatName);
			return;
		}
