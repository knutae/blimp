...
/**
 * A slider component for numerical values which is a composite of a caption label,
 * a edit field and a visual slider (scale).
...
public class ValueSlider extends Composite {
	...
	public ValueSlider(Composite parent, int style,
			String caption, int minValue, int maxValue, int numDigits) {
		super(parent, style);
		...
		//valueEdit.addListener(SWT.FocusOut, applyTextValueListener); // unreliable
		...
		captionLabel.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
	...
	public void setDigits(int digits) {
		...
		for (int i=0; i<digits; i++)
			mult *= 10;
		scale.setIncrement(mult);
		scale.setPageIncrement(mult);
		*/
	}
